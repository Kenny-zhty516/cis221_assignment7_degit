...
function App() {
  ...
  const [loading, setLoading] = useState(false); const [hasResult, setHasResult] = useState(false);
  ...
  const handleClick = async (event: MouseEvent<HTMLElement>) => {
    ...
    setHasResult(false);
    setLoading(true);
    const response = await fetch("https://openlibrary.org/search.json?q=Learning React");
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-start gap-4
  mt-20 text-center"> <div>
        <input
          type="text"
  ...
  rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          ...
          value={bookName} onChange={handleInputChange}
        /> </div>
      <button
        type="button"
        onClick={handleClick}
